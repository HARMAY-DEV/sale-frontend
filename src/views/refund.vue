  ...
  <div style="background: #f8f8f8; padding: 0; position: relative">
    <div class="refund" style="display: none">
      ...
      <div class="fun_btom">
        <div>退款信息：</div>
        <div>
          <span style="margin-right: 15px" v-for="(item, i) in items" :key="i"
            >{{ item.ptype }}: <i>￥{{ item.price }}</i></span
          >
        </div>
        <div>
          总计: <i>￥{{ funBt }}</i>
      ...
      <div class="refund_prop" v-if="prop">
        <div class="prop">
          ...
          <div style="color: rgba(142, 142, 142, 100)">
            已完成退款(2秒后自动关闭)
          </div>
        </div>
      </div>
    ...
    <div class="refund" style="height: 100%;">
    <div>
      <div class="refund_prop" v-if="prop">
        <div class="prop">
          <img src="../images/6.png" alt="" style="width: 60px" />
      ...
      <div @click="previous" class="top">
        <i
          ...
          style="font-weight: 600; font-size: 24px; position: absolute; left: 0"
        ></i>
        <p
          style="
            font-size: 19px;
      ...
      <div
        ...
        @click="refundMode(item, i)"
      >
        <img src="../images/5.png" class="listImg" alt="" />
        <div class="listRight">
          <div style="display: flex; align-items: center">
            ...
            <p style="font-size: 20px; color: #000000; font-weight: 600">
              ¥ {{ item.price }}
            </p>
          </div>
          <p
      ...
      <div class="footer">
        <div class="footerLeft">
          ...
          <div style="display: flex; align-items: center; margin-top: 18px">
            <p
              style="font-size: 16px; line-height: 22px; margin-right: 48px"
              v-for="(item, i) in items"
              :key="i"
            >
              {{ item.ptype }}：<span style="color: #b51f29"
                >¥{{ item.price }}
              </span>
            </p>
          </div>
          <p style="font-size: 16px; margin-top: 9px; line-height: 22px">
            总计：<span style="color: #b51f29">¥ {{ funBt }}</span>
          </p>
        </div>
        <el-button
          style="
  ...
  methods: {
    ...
    search() {
      if (this.searchGoodsNo.indexOf("https") == -1) {
        ...
        for (var i = 0; i < this.goodsTableData.length; i++) {
          if (
            this.goodsTableData[i].sn == "" ||
            this.goodsTableData[i].sn == null ||
            this.goodsTableData[i].sn == undefined
          ) {
            if (
              Number(this.searchGoodsNo) == Number(this.goodsTableData[i].id)
            ) {
    ...
    submit(value, num) {
      switch (value) {
        ...
        case "subtraction":
          if (
            this.goodsTableDatas.length == 0 &&
            Number(this.goodsTableData[num].quantity) <
              Number(this.historyList[num].quantity)
          ) {
            this.goodsTableData[num].quantity++;
            return;
          }
          if (
            this.goodsTableData[num].quantity >=
              this.historyList[num].quantity ||
            Number(this.goodsTableData[num].quantity) +
              Number(this.goodsTableDatas[num].quantity) >=
              Number(this.historyList[num].quantity)
          ) {
    ...
    products(data) {
      ...
      this.goodsTableData.forEach((v, i) => {
        if (!v.sn && v.id == this.goodsTableDatas[i].id) {
          v.quantity =
            this.historyList[i].quantity - this.goodsTableDatas[i].quantity;
        }
      });
