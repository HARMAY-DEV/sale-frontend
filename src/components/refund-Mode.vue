<template>
  <div class="refMode">
    ...
    >
      ...
      <span v-show="!item.commodity"
        >退款:
        <el-input-number
          v-model="refunValue[i].price"
          @change="refuchang"
          :min="0"
          :max="feMaxs(item)"
          label="描述文字"
...
export default {
  ...
  methods: {
    ...
    feMaxs(item) {
      let pro =
        this.orderTableDatas[0].price * this.orderTableDatas[0].quantity;
    ...
    chexSn() {
      ...
      this.refunValue.forEach((v, i) => {
        ...
        if (this.orderTableDatas[1][i]) {
          if (
            v.price >
            this.orderTableDatas[1][i].amount - this.$parent.items[i].price
          ) {
